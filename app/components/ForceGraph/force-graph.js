...
const linkedMethods = Object.assign(
  ...['d3Force', 'd3ReheatSimulation', 'emitParticle'].map((p) => ({ [p]: bindFG.linkMethod(p) }))
);
...
export default Kapsule({
    ...
    nodePointerAreaPaint: {
      onChange(paintFn, state) {
        state.shadowGraph.nodeCanvasObject(
          !paintFn ? null : (node, ctx, globalScale) => paintFn(node, node.__indexColor, ctx, globalScale)
        );
    ...
    linkPointerAreaPaint: {
      onChange(paintFn, state) {
        state.shadowGraph.linkCanvasObject(
          !paintFn ? null : (link, ctx, globalScale) => paintFn(link, link.__indexColor, ctx, globalScale)
        );
        state.flushShadowCanvas && state.flushShadowCanvas();
      },
      triggerUpdate: false,
  ...
  methods: {
    ...
    centerAt: function (state, x, y, transitionDuration) {
      ...
      if (x !== undefined || y !== undefined) {
        const finalPos = Object.assign({}, x !== undefined ? { x } : {}, y !== undefined ? { y } : {});
    ...
    getGraphBbox: function (state, nodeFilter = () => true) {
      ...
      return !nodesPos.length
        ? null
        : {
            x: [d3Min(nodesPos, (node) => node.x - node.r), d3Max(nodesPos, (node) => node.x + node.r)],
            y: [d3Min(nodesPos, (node) => node.y - node.r), d3Max(nodesPos, (node) => node.y + node.r)],
          };
    },
  ...
  init: function (domNode, state) {
    ...
    state.shadowCanvas.style.pointerEvents = 'none';
    state.shadowCanvas.style.opacity = 0.5;
    ...
    const getObjUnderPointer = () => {
      ...
      const px =
        pointerPos.x > 0 && pointerPos.y > 0
          ...
          : null;
      if (px && px.data && px.data[3] !== 0) {
      }
      // Lookup object per pixel color
      px && (obj = state.colorTracker.lookup(px.data));
      // if (obj) {
      //   console.log(px.data, obj.d)
      // }
      return obj;
    };
