...
const SelectMappingTargetHeader = observer((props: IHeaderProps) => {
  ...
  return (
    <div className={styles.panelTitle}>
      ...
      <Select
        bordered={false}
        className={cls(styles.configTargetSelect, {
          [styles.noValue]: !value,
          [styles.edgeLabel]: type === ISchemaEnum.Edge,
        })}
      ...
      >
        {targetList
          ?.filter((t) => !config.some((c) => c.name === t))
          .map((t) => (
  ...
  const resetFileSource = useCallback(
    (item: ITagFileItem | IEdgeFileItem, file: IImportFile) => {
      ...
      configItem.resetFileItem(
        index,
        configItem.type === ISchemaEnum.Tag ? new TagFileItem(payload) : new EdgeFileItem(payload),
      );
    },
    [configItem],
  );
