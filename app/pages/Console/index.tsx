      ...
      <div className="studioCenterLayout">
        <div className={styles.consolePanel}>
          <div className={styles.panelHeader}>
            <span className={styles.title}>{`${window.gConfig.databaseName
              } ${intl.get("common.console")}`}</span>
            <Icon
              className={styles.setting}
              type={"icon-studio-btn-consoleSetting"}
              ...
              <div className={styles.btnOperations}>
                {gpt.config.enableGPT2NGQLs && (
                  <Tooltip title={intl.get("console.gptIconTip")}>
                    ...
                    }} className={styles.btnOperations} type="icon-studio-btn-consoleGTP" />
                  </Tooltip>
                )}
                {gpt.config.enableCopilot && (
                  <Tooltip title={intl.get("console.copilotTip")}>
                    {!gpt.running ? (
                      <Icon onClick={() => { gpt.checkCopilotList(editor.current.editor) }} className={styles.btnOperations} type={"icon-studio-btn-consoleTip"} />
                    ) : (
          ...
          <div className={styles.codeInput}>
            ...
            />
            <Popover trigger={"click"} onOpenChange={(open) => {
              console.update({ showCompletion: open })
            }} placement='bottomLeft' open={console.showCompletion} key={`${pos.x}-${pos.y}`} rootClassName={styles.completions} content={
              <div className={styles.completionsContent}>
                {[...completionList, ...gpt.completionList].map((item, index) => (
                  ...
                  }} className={styles.completionsItem + " " + (index === activeCompletionIndex ? styles.active : "")} key={index} >
                    <div className={styles.completionsItemName}>{
                      item.type === "copilot" ? <CodeMirror value={item.text} options={{ mode: "nebula", lineWrapping: false, readOnly: true, lineNumbers: false, scrollbarStyle: "null" }} height='30' /> : <div className={styles.keywordTitle}>{item.text}</div>
                    }</div>
                    <div className={styles.completionsItemDesc}>{item.type}</div>
                  </div>))}
                {gpt.running && <div className={`${styles.completionsItem} ${styles.gptRunning}`}>
                  <LoadingOutlined className={styles.btnOperations} />GPT loading...
                </div>}
              </div>
            }>
              <span style={{ position: "absolute", top: pos.y, left: pos.x }}></span>
            </Popover>
          </div>
        </div>
