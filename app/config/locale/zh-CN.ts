export default {
  ...
  doc: {
    ...
    snsIntro:
      "社交网络示例图谱空间，包含用户、好友、帖子、评论等数据，新好友推荐、时间线生成、社交分析",
    movieIntro: "电影推荐图谱，尝试在图上做 CBF、ItemCF 和 UserCF 算法",
    datalineageIntro:
      "元数据管理实例数据集，血缘查询、血缘依赖分析、大数据治理",
    idMappingIntro: "ID Mapping，用户系统上的实体分析案例",
    fifa2022Intro:
      "面向关系而非复杂属性的世界杯图谱，尝试利用图算法预测冠军吧（假设我们还不知道冠军）",
    shareholdingIntro:
      "股权关系图谱，图上实做股权穿透、公司实控人分析、背景调查",
    openstackIntro:
      "图驱动的复杂基础设施智能运维实操，基于 OpenStack 全资源抓取图谱的例子",
  ...
  formRules: {
    ...
    ttlRequired:
      "请选择TTL指定的属性, 且属性的数据类型需为 integer 或 timestamp",
  ...
  console: {
    ...
    exportEdge:
      "请选择结果中分别代表边的起点（src_vid）、终点（dst_vid）和权重（rank）的列",
    deleteHistory: "清除历史",
    cypherParam: "自定义参数",
    favorites: "收藏夹",
    ...
    selectSpace: "请选择图空间",
    setting: "控制台设置",
    gptAPITooltip: '请使用chat/completioins api',
    gptModelVersion: '模型版本',
    features: "功能",
    docLength: "最大文档字符长度",
    useSpaceSchema: "使用图空间 Schema",
    useConsoleNGQL: "使用控制台 nGQL",
    send: "发送",
    explain: "解释",
    gptIconTip: "使用GPT解释当前语句",
    copilotTip: "使用GPT自动补全当前语句",
    copyGPT2NGQL:"复制到控制台"
  },
  explore: {
    vertexStyle: "节点颜色",
  ...
  import: {
    ...
    uploadBoxTip:
      "使用 YAML 配置文件用来描述待导入文件信息、数据库服务器等信息。",
    fileUploadRequired:
      "1. 请确保在导入 YAML 文件之前上传所有 CSV 数据文件。 如果没有，请先前往",
    ...
    fileSizeLimit:
      "{name}文件过大，超过上传限制({size})，请将文件通过 scp 的方式上传到安装目录下的 data/upload 目录",
    noHttp: "配置文件中的 address 不支持携带 http 协议，请去除 http(s)://",
    addressMatch:
      "配置文件中的 address 字段必须包含当前登录的 Graph 地址。多个地址用“,”隔开。",
    dataSourceFile: "文件源",
    vidColumn: "VID 列",
    ...
    retry: "重试次数",
    graphAddressTip:
      "Graph 服务的地址和端口。将使用以下 Graph 节点进行数据导入",
    ...
    datasourceMissing:
      "{files} 所在数据源未找到，请重新添加相关数据源并重新配置任务",
  ...
  schema: {
    ...
    fieldDisabled:
      "该属性被 ttl_col 引用，不支持更改操作，如要更改，请先更新 ttl",
    ...
    indexedLengthDescription:
      "设置索引字符串的长度。如果索引定长字符串，则索引长度无法修改。",
    ...
    dateFormat:
      "日期类型支持插入方式: <br /> 调用函数 date()，例如：date('2021-03-17')",
    timeFormat:
      "时间类型支持插入方式: <br /> 调用函数 time()，例如time('17:53:59')",
    datetimeFormat:
      "日期时间类型支持插入方式: <br /> 调用函数 datetime()，例如：datetime('2021-03-17T17:53:59')",
    geographyFormat:
      "geo 类型支持插入方式: <br /> 调用函数 ST_GeogFromText()，例如：ST_GeogFromText('POINT(6 10)')",
  ...
  link: {
    nGQLHref:
      "https://docs.nebula-graph.com.cn/3.5.0/3.ngql-guide/1.nGQL-overview/1.overview/",
    mannualHref:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/about-studio/st-ug-what-is-graph-studio/",
    startStudioHref:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/quick-start/st-ug-plan-schema/",
    versionLogHref:
      "https://docs.nebula-graph.com.cn/3.5.0/20.appendix/release-notes/studio-release-note/",
    ...
    loginHref:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/deploy-connect/st-ug-connect/",
  },
  sketch: {
    dragTip: "拖放到画布上",
    tag: "标签",
    ...
    sameSchemaWarning:
      "图空间中已存在{content}，请修改{hasType}名称，或者选择其他图空间。",
    noData: "暂无数据，请先导入数据",
  ...
  welcome: {
    ...
    schemaModuleLink:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/quick-start/st-ug-create-schema/",
    importModuleLink:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/quick-start/st-ug-import-data/",
    consoleModuleLink:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/quick-start/st-ug-console/",
    sketchModuleLink:
      "https://docs.nebula-graph.com.cn/3.5.0/nebula-studio/quick-start/draft/",
    basketballplayerDocLink:
      "https://nebula-graph.com.cn/posts/playground-basketball-player",
    ...
    fraudDetectionDocLink:
      "https://www.siwei.io/fraud-detection-with-nebulagraph/",
    fifa2022DocLink:
      "https://www.siwei.io/chatgpt-and-nebulagraph-predict-fifa-world-cup/",
    alwaysShow: "始终展示欢迎页",
